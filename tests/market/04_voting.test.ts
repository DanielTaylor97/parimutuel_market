...
describe("Place Votes", () => {
    ...
    it("Places votes", async () => {
        ...
        const skipOverTimeout = 10*60;   // 10 more minutes -- now just outside betting window
        simulateTimeTravel(client, skipOverTimeout);
        ...
        try {
            await program.methods.vote(marketParams, new anchor.BN(voteAmount3), voteDirection3, [...voterSignedMessage3])
                .accounts({ ...voter3Accounts})
                .signers([voter3, treasury])
                .rpc();
        
            assert(false, "Allowed second vote")
        } catch (err) {
            expect(err).to.be.instanceOf(AnchorError);
            expect((err as AnchorError).error.errorCode.number).to.equal(6706);
            expect((err as AnchorError).error.errorCode.code).to.equal("AlreadyVoted");
        }

        simulateTimeTravel(client, 1);

        // Bettor should not be allowed to vote
        try {
            await program.methods.vote(marketParams, new anchor.BN(bettor2voteAmount), bettor2voteDirection, [...bettor2SignedVotingMessage])
                .accounts({ ...bettor2votingAccounts})
                .signers([bettor2, treasury])
                .rpc();
        
            assert(false, "Allowed bettor to vote")
        } catch (err) {
            expect(err).to.be.instanceOf(AnchorError);
            expect((err as AnchorError).error.errorCode.number).to.equal(6705);
            expect((err as AnchorError).error.errorCode.code).to.equal("CannotVoteWithBets");
        }
        ...
        const totFor = voteAmount1*(voteDirection1 ? 1 : 0) + voteAmount2*(voteDirection2 ? 1 : 0) + voteAmount3*(voteDirection3 ? 1 : 0) + voteAmount4*(voteDirection4 ? 1 : 0) + voteAmount5*(voteDirection5 ? 1 : 0) + voteAmount6*(voteDirection1 ? 1 : 0)
        const totAgainst = voteAmount1*(voteDirection1 ? 0 : 1) + voteAmount2*(voteDirection2 ? 0 : 1) + voteAmount3*(voteDirection3 ? 0 : 1) + voteAmount4*(voteDirection4 ? 0 : 1) + voteAmount5*(voteDirection5 ? 0 : 1) + voteAmount6*(voteDirection1 ? 0 : 1);
